...
public class RoomNode
{
    ...
    public void MakePathInfo(RoomNode toNode)
    {
        ...
        if (eRelativeRectDirection.LEFT == relativeDirection)
        {
            ...
            fromDoor._doorPosition = new Vector3(from.xMin - 1, yPos, 0);
            toDoor._doorPosition = new Vector3(to.xMax, yPos, 0);
        }
        else if (eRelativeRectDirection.RIGHT == relativeDirection)
        {
            //to가 오른쪽 방
            int yPos = UnityEngine.Random.Range(doorYMin + 3, doorYMax - 3);
            fromDoor._doorPosition = new Vector3(from.xMax, yPos, 0);
            toDoor._doorPosition = new Vector3(to.xMin-1, yPos, 0);
        }
        else if (eRelativeRectDirection.DOWN == relativeDirection)
        {
            //to가 아래쪽
            int xPos = UnityEngine.Random.Range(doorXMin + 3, doorXMax - 3);
            fromDoor._doorPosition = new Vector3(xPos, from.yMin-1, 0);
            toDoor._doorPosition = new Vector3(xPos, to.yMax, 0);
        }
        else if (eRelativeRectDirection.UP == relativeDirection)
        {
            //위
            int xPos = UnityEngine.Random.Range(doorXMin + 3, doorXMax - 3);
            fromDoor._doorPosition = new Vector3(xPos, from.yMax, 0);
            toDoor._doorPosition = new Vector3(xPos, to.yMin-1, 0);
        }
